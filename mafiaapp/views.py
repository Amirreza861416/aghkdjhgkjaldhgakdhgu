...
def index(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'mafiaapp/firstlogin.html')

def games(request):
    return render(request, 'mafiaapp/game1.html')
# /game_id/
    ...
    if request.method == 'POST':
        ...
        if (jsondata.get('code')):
            ...
            try:
                ...
                ret['status'] = 'failed'
                ret['message'] = 'همچین بازی ای وجود ندارد'
                return JsonResponse(ret)
            try:
                if (game.user_set.get(name=name)):
                    ret['status'] = 'failed'
                    ret['message'] = 'فردی با اکانت شما در بازی است'
                    return JsonResponse(ret)
            except Exception:
    ...
    if request.method == 'POST':
        try:
            ...
            if user_list.count() < 5:
                ret['status'] = 'failed'
                ret['message'] = 'حداقل باید 5 نفر در بازی باشند'
                return JsonResponse(ret)
            if 'user' in request.session:
                username = request.session['user']
                user = user_list.get(name=username)
                if user.creator == False:
                    ret = returnfail(ret, 'سازنده بازی باید بازی را شروع کند')
            else:
...
def randomcode(chars=string.ascii_uppercase + string.digits):
